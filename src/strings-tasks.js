...
function removeFirstOccurrences(str, value) {
  ...
  return (
    str.substring(0, find) + str.substring(find + value.length, str.length)
  );
}
...
function removeLastOccurrences(str, value) {
  const find = str.lastIndexOf(value);
  if (find === -1) {
    return str;
  }
  return (
...
function isPalindrome(str) {
  const strFormatted = str.replace(/[?!, ]/g, '').toLowerCase();
  if (strFormatted.split('').reverse().join('') === strFormatted) {
    return true;
  }
  return false;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    const charAsNumber = str.charCodeAt(i);
    if (charAsNumber >= 65 && charAsNumber <= 90) {
      result += currentLetter.toLowerCase();
    } else if (charAsNumber >= 97 && charAsNumber <= 122) {
...
function encodeToRot13(str) {
  const array1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz ?!,;:';
  const array2 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm ?!,;:';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    const indexA1 = array1.indexOf(str[i]);
    result += array2.charAt(indexA1);
  }
  return result;
}
